...
//        this.verifyBehaviorCaptcha(req.getCaptchaToken());
    ...
    public PageResp<CouponMyCouponResp> listMyCoupons(CouponMyCouponQuery query, PageQuery pageQuery) {
        ...
        IPage<CouponUserCouponDO> page = userCouponMapper.selectPage(
        ...
        // 填充活动名称和模板名称
        Set<Long> activityIds = page.getRecords().stream()
        ...
        Map<Long, CouponActivityDO> activityMap = CollUtil.isEmpty(activityIds) ? Collections.emptyMap()
                : activityMapper.selectBatchIds(activityIds).stream()
                        .collect(Collectors.toMap(CouponActivityDO::getId, a -> a));

        Map<Long, CouponTemplateDO> templateMap = CollUtil.isEmpty(templateIds) ? Collections.emptyMap()
    ...
    public Long writeOff(CouponWriteOffReq req) {
        ...
        CouponUserCouponDO userCoupon = userCouponMapper.selectOne(new LambdaQueryWrapper<CouponUserCouponDO>()
                ...
                .eq(CouponUserCouponDO::getIsDeleted, 0));
        CheckUtils.throwIfNull(userCoupon, "券码不存在");
        CheckUtils.throwIfNotEqual(CouponConstants.USER_COUPON_STATUS_UNUSED, userCoupon.getStatus(),
                "券状态不允许核销");
        ...
        if (CouponConstants.WRITE_OFF_STATUS_APPROVED.equals(writeOff.getStatus())) {
            templateMapper.update(null, new LambdaUpdateWrapper<CouponTemplateDO>()
                    .eq(CouponTemplateDO::getId, template.getId())
                    .setSql("write_off_stock = write_off_stock + 1"));
        }

        return writeOff.getId();
    }

    private void saveSubmissionValues(Long submissionId, Long formTemplateId,
            List<CouponWriteOffReq.FieldValueReq> fieldValues) {
        // 查询字段映射
        Map<String, CouponFormFieldDO> fieldMap = formFieldMapper
    ...
    public PageResp<CouponWriteOffListResp> listWriteOffs(CouponWriteOffQuery query, PageQuery pageQuery) {
        ...
        IPage<CouponWriteOffDO> page = writeOffMapper.selectPage(
                new Page<>(pageQuery.getPage(), pageQuery.getSize()), wrapper);
    ...
    public Long resubmit(Long writeOffId, CouponWriteOffReq req) {
        ...
        CheckUtils.throwIfNotEqual(CouponConstants.WRITE_OFF_STATUS_REJECTED, writeOff.getStatus(),
                "仅驳回状态可重新提交");
    ...
    public void review(Long writeOffId, CouponReviewReq req) {
        ...
        CouponWriteOffDO writeOff = writeOffMapper.selectById(writeOffId);
        CheckUtils.throwIfNull(writeOff, "核销记录不存在");
        CheckUtils.throwIfNotEqual(CouponConstants.WRITE_OFF_STATUS_PENDING_AUDIT, writeOff.getStatus(),
                "当前状态不允许审核");
        ...
        if (Boolean.TRUE.equals(req.getApproved())) {
            ...
            // 更新模板核销数量
            templateMapper.update(null, new LambdaUpdateWrapper<CouponTemplateDO>()
                    .eq(CouponTemplateDO::getId, writeOff.getTemplateId())
                    .setSql("write_off_stock = write_off_stock + 1"));
    ...
    private void createReviewIssues(CouponWriteOffDO writeOff, CouponWriteOffSubmissionDO submission,
    ...
    private List<Long> getAllowedActivityIds(Long reviewerUserId) {
        if (UserContextHolder.isSuperAdmin()) {
            return activityMapper.selectList(new LambdaQueryWrapper<CouponActivityDO>()
    ...
    private CouponWriteOffDetailResp buildWriteOffDetailResp(CouponWriteOffDO writeOff) {
        ...
        if (writeOff.getCurrentSubmissionId() != null) {
            CouponWriteOffSubmissionDO submission = submissionMapper.selectById(writeOff.getCurrentSubmissionId());
            if (submission != null) {
                CouponWriteOffDetailResp.SubmissionResp submissionResp = BeanUtil.copyProperties(submission,
                        CouponWriteOffDetailResp.SubmissionResp.class);
                submissionResp.setSubmissionId(submission.getId());

                // 查询字段值
                if (template != null && template.getFormTemplateId() != null) {
                    List<CouponFormFieldDO> fields = formFieldMapper.selectList(
                            new LambdaQueryWrapper<CouponFormFieldDO>()
                                    .eq(CouponFormFieldDO::getTemplateId, template.getFormTemplateId())
                                    .orderByAsc(CouponFormFieldDO::getGroupSort)
                                    .orderByAsc(CouponFormFieldDO::getSortNo));
                    ...
                    List<CouponWriteOffSubmissionValueDO> values = submissionValueMapper.selectList(
                            new LambdaQueryWrapper<CouponWriteOffSubmissionValueDO>()
                                    .eq(CouponWriteOffSubmissionValueDO::getSubmissionId, submission.getId())
                                    .orderByAsc(CouponWriteOffSubmissionValueDO::getFieldId,
                                            CouponWriteOffSubmissionValueDO::getValueSeq));
                ...
                // 查询驳回问题
                List<CouponWriteOffReviewIssueDO> issues = reviewIssueMapper.selectList(
                        new LambdaQueryWrapper<CouponWriteOffReviewIssueDO>()
                ...
                if (CollUtil.isNotEmpty(issues)) {
                    Map<Long, CouponFormFieldDO> fieldMap = formFieldMapper.selectList(
                            new LambdaQueryWrapper<CouponFormFieldDO>()
                                    .eq(CouponFormFieldDO::getTemplateId, template.getFormTemplateId()))
                            .stream()
                            .collect(Collectors.toMap(CouponFormFieldDO::getId, f -> f));
                    ...
                    submissionResp.setIssues(issues.stream().map(issue -> {
                        CouponWriteOffDetailResp.ReviewIssueResp issueResp = BeanUtil.copyProperties(issue,
                                CouponWriteOffDetailResp.ReviewIssueResp.class);
